...
void PWM_Init(void)
{
    ...
    */
    ...
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; // 使用复用推挽输出，因为输出源来自TIM2_CHx
    ...
    TIM_TimeBaseStructure.TIM_Period = 100 - 1;                 // ARR寄存器的值，设定计数周期，到这个时间清零重新开始
    TIM_TimeBaseStructure.TIM_Prescaler = 720 - 1;              // PSC寄存器的值，分频的系数
